...
pipeline {
   ...
   environment{
    ...
    stage('Run python inventory creator and Apigee Env Creation Ansible Script') {
        steps {
          script {
              sh """
              sh $WORKSPACE/apigee-installation/install/install_env_creation.sh $CLUSTER $CLIENT_TOKEN """
          }
        }
      }
   }
