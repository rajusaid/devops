...
pipeline {
   ...
   stage ('Replace password placeholders') {
    ...
    stage('Run python inventory creator and OpenShift Env Creation Ansible Script') {
        steps {
          script {
              sh """
              sh $WORKSPACE/openshift-secret-router/install/env_prefix_install_env_creation.sh $CLUSTER $ENV_PREFIX $CLIENT_TOKEN 
              """
          }
        }
      }
  ...
  post{
    always{
      script{
        sh '''
          sudo rm -rf /tmp/openshift/cluster/$CLUSTER
